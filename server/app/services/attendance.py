...
def _parse_date(v) -> date_type | str:
...
def create(db: Session, dto: CreateAttendanceDto):
    ...
    parsed_date = _parse_date(dto.date)
    existing = db.query(Attendance).filter(Attendance.student_id == dto.studentId, Attendance.date == parsed_date).first()
    if existing:
    ...
    rec = Attendance(student_id=dto.studentId, date=parsed_date, status=dto.status.value if hasattr(dto.status, 'value') else dto.status)
    db.add(rec)
    db.commit()
    db.refresh(rec)
    return {"id": rec.id, "studentId": rec.student_id, "student_id": rec.student_id, "date": str(rec.date), "status": rec.status}
