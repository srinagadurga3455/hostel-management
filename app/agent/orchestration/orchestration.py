...
async def orchestrate(message: str, session_id: str | None, user_id: str, role: str, token: str | None):
    ...
    if agent_plan.requires_confirmation or requires_confirmation(agent_plan.tool_name):
        ...
        return {"success": True, "message": f"You are requesting {agent_plan.intent.lower().replace('_',' ')} ({args_str}). Should I submit it? Please reply Yes to confirm or No to cancel.", "session_id": session.session_id, "tool_used": agent_plan.tool_name, "requires_confirmation": True}
